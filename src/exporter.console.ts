...
export function exporter(trace: rian.Trace) {
    ...
    for (let scope of trace.scopeSpans) {
        ...
        const [max_time, min_time] = spans.reduce((col, span) => {
        ...
        for (let span of spans) {
            ...
            let start = Math.ceil((span.start - min_time) / (max_time - min_time) * max_trace_col);
            end = end ? Math.ceil((span.end! - min_time) / (max_time - min_time) * max_trace_col) : start
